...
(let ((gcontext
  ...
  (defun default-gcontext (drawable default)
    "Return a gcontext with the same attributes as DEFAULT which can
     ...
     return value is the cache to be searched."
    (declare
...
(defparameter *gcontext-test-sequence*
  ...
  "A permutation of gcontext state indexes, in order of
   'most-likely-to-differ-first'. Use this order to test if two
   gcontext states are equal, in order to detect inequality quickly.")

(defun find-gcontext (drawable match cache &rest initargs)
  "Return a gcontext for the DRAWABLE that matches the MATCH
   gcontext. Return an existing element of the gcontext CACHE or
   create and return a new CACHE element. The INITARGS, if given,
